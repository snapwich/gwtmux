...
_gwtmux_check_deps() {
    ...
    # Optional: warn about gh if not installed (enhances PR number support)
    if ! command -v gh >/dev/null 2>&1; then
        if [[ -z "$_GWTMUX_GH_WARNED" ]]; then
            echo "gwtmux: gh (GitHub CLI) not found - PR number support disabled" >&2
            echo "Install with: brew install gh" >&2
            export _GWTMUX_GH_WARNED=1
        fi
    fi
}
